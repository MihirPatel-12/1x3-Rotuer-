...
always@(posedge clk)
	begin
		if(!resetn)
			...
			begin 
				// FIX: changed from a second independent "if" to "else if" so
				// a live read (read_enb && !empty) can never be clobbered by
				// the "count==0" high-Z condition in the same cycle. Without
				// this, the very first byte (header) read of every packet -
				// read at the exact cycle count is still 0 - was silently
				// overwritten with 8'bz.
				if(read_enb && !empty)
